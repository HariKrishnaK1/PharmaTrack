...
export const Login = () => {
  ...
  const handleQuickDemo = (role) => {
    ...
    try {
      ...
      toast.success(`Logged in as ${demoUser.name} (Demo Mode - View Only)`);
      navigate('/app');
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-2xl border border-slate-200 relative z-10">
        ...
        <div className="pt-4 border-t border-slate-100">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider text-center mb-1 flex items-center justify-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5 text-teal-600" /> One-Click Demo Mode (View-Only)
          </div>
          <p className="text-[10px] text-slate-400 text-center mb-3">
            Explore features with simulated mock data. Real database is protected.
          </p>
          <div className="grid grid-cols-3 gap-2 text-center">
            <button
              type="button"
              onClick={() => handleQuickDemo('ADMIN')}
              className="px-2 py-2 rounded-lg bg-purple-50 text-purple-800 border border-purple-200 text-xs font-semibold hover:bg-purple-100 transition"
            >
              demoAdmin
              <span className="block text-[9px] text-purple-600 font-normal mt-0.5">Admin View</span>
            </button>
            <button
              type="button"
              onClick={() => handleQuickDemo('INVENTORY_MANAGER')}
              className="px-2 py-2 rounded-lg bg-blue-50 text-blue-800 border border-blue-200 text-xs font-semibold hover:bg-blue-100 transition"
            >
              demoInventory
            ...
            <button
              ...
              className="px-2 py-2 rounded-lg bg-teal-50 text-teal-800 border border-teal-200 text-xs font-semibold hover:bg-teal-100 transition"
            >
              demoWarehouse
