...
export const Navbar = ({ isCollapsed, onToggleMobile, onOpenSearch, onOpenAI }) => {
  const { user, logout, demoLogin, isDemo, hasRealAccount, realUser, restoreRealAccount } = useAuth();
    ...
    <header className="sticky top-0 z-30 flex items-center justify-between h-16 px-4 md:px-6 bg-white border-b border-slate-200/80 shadow-xs">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative" ref={roleMenuRef}>
          ...
          {showRoleMenu && (
            <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden z-50 p-2 text-xs">
              {isDemo && hasRealAccount && (
                <div className="mb-2 pb-2 border-b border-slate-100">
                  <button
                    ...
                    className="w-full p-2.5 rounded-lg bg-teal-50 border border-teal-200 hover:bg-teal-100 transition text-left flex items-center justify-between group"
                  >
                    <div>
                      <div className="font-bold text-xs text-teal-900 flex items-center gap-1.5">
                        <UserCheck className="w-3.5 h-3.5 text-teal-700" /> Return to My Real Account
                      </div>
              ...
              <div className="px-2 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                {hasRealAccount ? 'Switch Role Perspective' : 'Simulate Role View'}
              </div>

              <button
                onClick={() => handleRoleSwitch('ADMIN')}
                className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg transition text-left ${
                  user?.role === 'ADMIN' ? 'bg-purple-50/80 border border-purple-200' : 'hover:bg-slate-50'
                }`}
              >
                <div>
                  <div className="font-semibold text-slate-900 flex items-center gap-1.5">
                    Administrator
                    {realUser?.role === 'ADMIN' && (
                      <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-purple-100 text-purple-800">Your Account</span>
                    )}
                  </div>
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg transition text-left ${
                  user?.role === 'INVENTORY_MANAGER' ? 'bg-blue-50/80 border border-blue-200' : 'hover:bg-slate-50'
                }`}
              >
                <div>
                  <div className="font-semibold text-slate-900 flex items-center gap-1.5">
                    Inventory Manager
                    {realUser?.role === 'INVENTORY_MANAGER' && (
                      <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-blue-100 text-blue-800">Your Account</span>
                    )}
                  </div>
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg transition text-left ${
                  user?.role === 'WAREHOUSE_MANAGER' ? 'bg-teal-50/80 border border-teal-200' : 'hover:bg-slate-50'
                }`}
              >
                <div>
                  <div className="font-semibold text-slate-900 flex items-center gap-1.5">
                    Warehouse Manager
                    {realUser?.role === 'WAREHOUSE_MANAGER' && (
                      <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-teal-100 text-teal-800">Your Account</span>
                    )}
                  </div>
